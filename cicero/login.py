...
class LoginApp:
    ...
    def load_service_account_file(self):
        ...
        try:
            cicero.config.gcloud_credentials = google.oauth2.service_account.Credentials.from_service_account_file(file)
            cicero.config.gcloud_credentials_scoped = cicero.config.gcloud_credentials.with_scopes(scope)
    ...
    def check_service_account_file(self):
        self.translate_client = google.cloud.translate_v2.Client(credentials=cicero.config.gcloud_credentials)
        try:
            self.translate_client.translate("TEST", target_language="EN")
            cicero.config.has_logged_in = True
            cicero.config.gcloud_translate_client = self.translate_client
            ...
            cicero.config.has_logged_in = False
    ...
    def login_into_main_app(self):
        ...
        if cicero.config.gcloud_credentials is not None:
